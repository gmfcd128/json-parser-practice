...
static const char* lept_parse_hex4(const char* p, unsigned* u) {
	...
	for (int i = 0; i < 4; i++) {
		if ('0' <= p[i] && p[i] <= '9') {
			result += (p[i] - '0') * pow(16, 3 - i);
		}
		else if ('a' <= p[i] && p[i] <= 'f') {
			result += (p[i] - 'a' + 10) * pow(16, 3 - i);
		}
		else if ('A' <= p[i] && p[i] <= 'F') {
			result += (p[i] - 'A' + 10) * pow(16, 3 - i);
		}
		...
		case '\\':
			...
			case 'u':
				...
				if (0xd800 <= u && u <= 0xdbff) {
					p += 2;
					unsigned lower;
					if (!(p = lept_parse_hex4(p, &lower)))
						STRING_ERROR(LEPT_PARSE_INVALID_UNICODE_SURROGATE);
					if (0xdc00 <= lower && lower <= 0xdfff) {
						unsigned codepoint = 0x10000 + (u - 0xD800) * 0x400 + (lower - 0xDC00);
						u = codepoint;
					}
					else {
						STRING_ERROR(LEPT_PARSE_INVALID_UNICODE_SURROGATE);
